...
if __name__ == "__main__":
    cidades, matriz_distancias = ler_tsplib("brazil58.tsp")
    melhor_rota_tabu, melhor_custo_tabu = busca_tabu_tsp(matriz_distancias)


    resultados_tabu = realizar_experimentos_tabu(matriz_distancias)


    sns.boxplot(data=resultados_tabu)
    plt.axhline(melhor_solucao_conhecida, color='red', linestyle='--', label=f'Melhor Solução Conhecida ({melhor_solucao_conhecida})')
    plt.title('Distribuição dos Custos das Soluções Geradas pela Busca Tabu')
