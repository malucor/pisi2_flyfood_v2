...
def plotar_rota(melhor_rota, coordenadas):
    x = [coordenadas[i][0] for i in melhor_rota] + [coordenadas[melhor_rota[0]][0]]
    y = [coordenadas[i][1] for i in melhor_rota] + [coordenadas[melhor_rota[0]][1]]
    ...
    for i, (xi, yi) in enumerate(coordenadas):
        plt.text(xi, yi, str(i), fontsize=12, ha='right', color="blue")

    plt.title("Melhor Rota Encontrada - ACO")
    plt.grid(True)
    plt.legend()
    plt.show()
...
if __name__ == "__main__":
    ...
    print("Melhor rota (ACO):", melhor_rota_aco)
    print("Custo (ACO):", melhor_custo_aco)

    plotar_rota(melhor_rota_aco, cidades)